...
import { Receipt, DetailedReceipt } from "@/types/Receipt";
import { fetchReceiptDetails } from "@/app/queries/get-receipt-details";
import { AuthTokens } from "@/types/AuthTokens";

export default function ReceiptList({receipts, tokens, setTokens}: {receipts: Receipt[], tokens: AuthTokens, setTokens: (tokens: AuthTokens | null) => void}) {
  ...
  async function getReceipt(transactionId: string) {
    const data = await fetchReceiptDetails(transactionId, tokens, setTokens)
    if (data) {
      openModal(data);
    }
    }
  ...
  <h2 className="text-xl font-semibold mb-4 col-span-2">Your Receipts</h2>
    ...
    {receipts.map((receipt) => (
      ...
      >
        ...
        <CardContent className="flex justify-between items-center">
          <div>
            <p>Total: {formatAmount(receipt.total.amount.amount)}</p>
